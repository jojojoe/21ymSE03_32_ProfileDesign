...
@implementation PictureProcessTool
...
static CGContextRef CreateRGBABitmapContext (CGImageRef inImage)// 返回一个使用RGBA通道的位图上下文
{
    ...
    CGFloat pixelsWide = CGImageGetWidth(inImage); //获取横向的像素点的个数
    CGFloat pixelsHigh = CGImageGetHeight(inImage); //纵向
...
+ (UIImage*)imageWithImage:(UIImage*)inImage withColorMatrix:(const float*) f
{
    ...
    CGFloat w = CGImageGetWidth(inImageRef);
    CGFloat h = CGImageGetHeight(inImageRef);
